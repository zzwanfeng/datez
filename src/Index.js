...
function datez (...argu) {
  return new datez.prototype.init(...argu);
}
function getInstance (that) {
  return that instanceof datez ? that : datez(that);
}
function isObject (value) {
  return value != null && (typeof value == 'object' || typeof value == 'function');
}

datez.setLanguage = function (lang, data = {}) {
  _langMap[lang] = Object.assign(_langMap[lang] || {}, data);
  return this;
};
datez.switchLanguage = function (lang) {
  _lang = lang;
  return this;
};
datez.now = Date.now;
datez.supportedTimezones = typeof Intl != 'undefined' && Intl.supportedValuesOf ? Intl.supportedValuesOf('timeZone') : [];
datez.switchTimezone = function (timezone) {
  _timezone = timezone;
  _offset = convertTimeZone(new Date('1970/1/1'), _timezone).getTime() - (new Date('1970/1/1')).getTime();
};
datez.getTimezone = function () {
  return _timezone;
};
datez.getTimezoneOffset = function () {
...
datez.prototype = {
  ...
  startOf (unit) {
    ...
    if (unit == 'timestamp') {
      ...
      that = datez($.year, $.month, $.day - $.week, 0, 0, 0, 0);
    } else {
      that = datez(...dateSet);
    }
    return that;
...
datez.prototype.init.prototype = datez.prototype;
